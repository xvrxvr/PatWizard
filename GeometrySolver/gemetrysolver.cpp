    ...
    bool GemetrySolver::ConstrainChecker(Constrain constr){
        ...
        switch (constr.type) {
            ...
            case Constrain::PointToPoint:
                //Check distance between points
                //Distance between points sqrt((x-x)^2 - (y-y)^2)
            ...
            case Constrain::LineToPoint:
                ...
                //Distance between point and line sqrt((x-x)^2 - (y-y)^2), where (x,y) - requered point and (x,y) - point of intersection point of line and normal
                //Сoefficients of equation of line
                A1 = obj2.y - obj2.y;
                B1 = (-1)*(obj2.x - obj2.x);
                C1 = (-1)*(obj2.x*(obj2.y - obj2.y) + obj2.y*(obj2.x - obj2.x));
            ...
            case Constrain::LineToLine:
                ...
                C1 = (-1)*(obj1.x*(obj1.y - obj1.y) + obj1.y*(obj1.x - obj1.x));
                C2 = (-1)*(obj2.x*(obj2.y - obj2.y) + obj2.y*(obj2.x - obj2.x));
                A1 = obj2.y - obj2.y;
                B1 = (-1)*(obj2.x - obj2.x);
            ...
            case Constrain::LineAngle:
                ...
                current_dist = atan((obj1.y - obj1.y)/(obj1.x-obj1.x));
                break;
            case Constrain::ArcToArc:
                ...
                else
                {
                    ...
                    rad1 = sqrt(pow(obj1.x - center1.x,2) + pow(obj1.y - center1.y, 2));
                    rad2 = sqrt(pow(obj1.x - center1.x,2) + pow(obj1.y - center1.y, 2));
                    //Check radius for obj 2
                    rad3 = sqrt(pow(obj2.x - center2.x,2) + pow(obj2.y - center2.y, 2));
                    rad4 = sqrt(pow(obj2.x - center2.x,2) + pow(obj2.y - center2.y, 2));
                    if ( rad1 != rad2 ||  rad3 != rad4 )
                        exit( -1);
                }
                //Distance between points sqrt((x-x)^2 - (y-y)^2)
                current_dist = sqrt(pow(center1.x - center2.x,2) + pow(center1.y - center2.y, 2));

                //Check crossing of line center-to-center and arcs
                angle1 = atan((center2.y - center1.y)/(center2.x-center1.x));//arc1
                angle2 = atan((obj1.y - center1.y)/(obj1.x-center1.x));
                angle3 = atan((obj1.y - center1.y)/(obj1.x-center1.x));
                //For  arc2
                angle4 = atan((center1.y - center2.y)/(center1.x-center2.x));
                angle5 = atan((obj2.y - center2.y)/(obj2.x-center2.x));
                angle6 = atan((obj2.y - center2.y)/(obj2.x-center2.x));
                ...
                {
            current_dist = min( sqrt(pow(obj1.x - center2.x,2) + pow(obj1.y - center2.y, 2)),
                                sqrt(pow(obj1.x - center2.x,2) + pow(obj1.y - center2.y, 2)))
                                        - rad3;
                }
                else if ( angle4 >= angle5 && angle4 <= angle6 )
                {
            current_dist = min( sqrt(pow(obj2.x - center1.x,2) + pow(obj2.y - center1.y, 2)),
                                sqrt(pow(obj2.x - center1.x,2) + pow(obj2.y - center1.y, 2)))
                                        - rad1;
                }
                else
                {
                   double dis1 = min( sqrt(pow(obj2.x - obj1.x,2) + pow(obj2.y - obj1.y, 2)),
                                sqrt(pow(obj2.x - obj1.x,2) + pow(obj2.y - obj1.y, 2)));
                   double dis2 = min( sqrt(pow(obj2.x - obj1.x,2) + pow(obj2.y - obj1.y, 2)),
                                sqrt(pow(obj2.x - obj1.x,2) + pow(obj2.y - obj1.y, 2)));
            ...
            case Constrain::ArcSweep:
                ...
                if ( obj1.type == 3 && obj2.type == 3 )/*ArcTo */
                {
                    ...
                    current_dist = acos((obj1.x-obj1.x)*(obj2.x-obj2.x)+
                                          (obj1.y-obj1.y)*(obj2.y-obj2.y)/
